# The Slow Motherhood Weekly Planner

*A 4-week undated planner for the mom who's tired of optimizing. 36 pages, designed to be used badly, on the days you remember.*

---

## How this planner is different (page 1)

Most planners are productivity systems for moms who don't have time for productivity systems.

This isn't that.

There are no time-blocked schedules. No "morning routine" prescriptions. No "5 things to do before 8am." No habit-tracker grid you'll abandon on day three and feel bad about.

What this planner has instead:

- **Four weekly spreads.** One page per week. Not seven.
- **Three questions a day, not twenty.** Most days you'll answer one. That's enough.
- **A monthly reflection** that doesn't ask if you "crushed your goals." It asks where you found two minutes of yourself this month.
- **An intention exercise** at the start that you can do in 15 minutes. Or break into 5-minute pieces. Or skip and come back to. The planner won't notice.

You don't have to use every page. You don't have to use them in order. You don't have to feel bad about the pages you skip.

Slow motherhood doesn't mean doing less. It means refusing to perform what you're not actually doing.

Use it badly. That's how this works.

— *the brand*

---

## Page 2 — A note on "slow"

A friend asked me what "slow motherhood" means. Here's what I said:

It's not a Sunday in a sunbeam with a clean linen apron and a sourdough loaf.

It's a Tuesday at 4pm when the laundry is still in the dryer and the baby is finally asleep and I have to choose between sitting with my coffee for 4 minutes or starting dinner — and I pick the coffee.

That's slow motherhood. The 4 minutes you choose yourself, when there's an equally valid case for the laundry.

Some weeks you'll pick the laundry every time. That's okay too. Slow is not a rule. It's a permission.

This planner is the permission, written down.

---

## Page 3 — Before you start: 5-minute intention setting

> *Sit somewhere. Anywhere. Bring a pen. Don't think too hard about the answers.*

### One word

If this season of motherhood had a single word — not the word you should pick, the word you'd pick if no one was reading — what's the word?

```
___________________________________________________
```

### What you actually want

Not a vision board. Not a 5-year plan.

Just one thing you wish were true about your day-to-day in 4 weeks that isn't true right now:

```
___________________________________________________
___________________________________________________
```

### What you can let go of

One thing you're doing because you "should" — not because it's working for you. Permission slip, signed by you, to drop it for the next 4 weeks:

```
___________________________________________________
___________________________________________________
```

### One small yes

One small thing that brought you joy in the last 7 days — even if it lasted 30 seconds. Notice it. Plan one of these for this week if you can:

```
___________________________________________________
```

### A note for your future self

If you read this in 4 weeks and feel like you "failed" the planner — what would you want to tell yourself now?

```
___________________________________________________
___________________________________________________
___________________________________________________
```

---

## Pages 4–11 — Week 1 spread

### Page 4 — Week 1 cover

> *This week's invitation*

What if this week, the only goal was: **[fill in one thing]**?

```
___________________________________________________
___________________________________________________
```

The week ahead in three words (not adjectives, not goals — just what's coming):

```
___________________   ___________________   ___________________
```

If only one thing happened this week, what would you want it to be?

```
___________________________________________________
```

What's already on the calendar that's not up for negotiation?

```
___________________________________________________
___________________________________________________
```

Three things you don't have to do this week even though someone might think you should:

```
1. ________________________________________________
2. ________________________________________________
3. ________________________________________________
```

---

### Pages 5–11 — Daily pages (7 of them, one per day)

Each day is one page. Each page has:

```
─────────────────────────────────────────────────────────────────

[ DAY OF WEEK ]                                  [ DATE ___/___ ]

─────────────────────────────────────────────────────────────────

THE TWO QUESTIONS

What's one thing for me today (even tiny)?
─────────────────────────────────────────────────────────────────

What's one thing for the kids today (it can be small)?
─────────────────────────────────────────────────────────────────

─────────────────────────────────────────────────────────────────

THE LIST (do or don't — the planner doesn't care)

☐ ________________________________________________
☐ ________________________________________________
☐ ________________________________________________

─────────────────────────────────────────────────────────────────

ONE NOTE FROM TODAY (a word, a phrase, a line of dialogue)

___________________________________________________
___________________________________________________

─────────────────────────────────────────────────────────────────

HOW I'D RATE TODAY (circle one, no shame)

🙂 mostly okay     😐 fine, that's enough     🌧️ today was hard

─────────────────────────────────────────────────────────────────
```

The intentional design choice: there's space for 3 list items, not 10. Three is the max you should plan in a day with small kids. If you wrote 10, you were lying to yourself. The planner won't help you lie to yourself.

---

### Page 12 — Week 1 reflection

> *Sit somewhere. 5 minutes. No phone.*

The smallest moment this week that felt right:

```
___________________________________________________
___________________________________________________
```

The thing I assumed I'd do and didn't:

```
___________________________________________________
```

How I feel about not doing it (one word):

```
___________________________________________________
```

If I could send next-week-me one piece of advice:

```
___________________________________________________
___________________________________________________
```

What's worth carrying into week 2:

```
___________________________________________________
___________________________________________________
```

---

## Pages 13–20 — Week 2 spread

*(Same structure as week 1. Pre-filled prompts subtly rotate so it doesn't feel identical.)*

### Page 13 — Week 2 cover variations

> *This week's invitation*

What if this week, you didn't try to "catch up"?

```
___________________________________________________
___________________________________________________
```

One thing you're noticing about being a mom this week that no one tells you:

```
___________________________________________________
___________________________________________________
___________________________________________________
```

What's the version of "rest" that's actually possible this week?

```
___________________________________________________
```

### Pages 14–20 — Daily pages, week 2

Same template as week 1, but the "TWO QUESTIONS" prompts rotate:

**Day 8:**
> What's something you'd do if you weren't worried about looking like a "good mom"?

**Day 9:**
> What's a small kindness you can offer yourself today that costs nothing?

**Day 10:**
> What did your body need today? (Did it get it?)

**Day 11:**
> What's a question your kid asked recently that's actually a real question?

**Day 12:**
> What's a moment from today you don't want to forget?

**Day 13:**
> What's something a past version of you would be surprised you handled?

**Day 14:**
> Where in your day was there a quiet 60 seconds? Did you notice?

---

### Page 21 — Week 2 reflection

What I let go of this week (intentionally or accidentally):

```
___________________________________________________
___________________________________________________
```

What I expected to feel guilty about and didn't:

```
___________________________________________________
```

A small thing that worked surprisingly well:

```
___________________________________________________
___________________________________________________
```

If I had a friend in my shoes, what would I tell her right now?

```
___________________________________________________
___________________________________________________
___________________________________________________
```

---

## Pages 22–29 — Week 3 spread

### Page 22 — Week 3 cover

> *This week's invitation*

What if your slowest day this week was actually the most successful?

Three things that are NOT measures of a good week, but feel like they are:

```
1. ________________________________________________
2. ________________________________________________
3. ________________________________________________
```

What's the actual measure that matters to you?

```
___________________________________________________
___________________________________________________
```

### Pages 23–29 — Daily pages, week 3

Rotated daily prompts:

**Day 15:**
> What's something that used to feel important and doesn't anymore?

**Day 16:**
> Where did your kid surprise you today?

**Day 17:**
> What's one thing you did today that nobody saw?

**Day 18:**
> When did you breathe deeply today (not because you were stressed — because you remembered)?

**Day 19:**
> If today was the only day someone watched a film of your life, would you be okay with it? (No right answer.)

**Day 20:**
> What's one thing that took longer than you wanted — and it was fine?

**Day 21:**
> What did you let be "good enough" today?

---

### Page 30 — Week 3 reflection

What this week showed me about what I actually need:

```
___________________________________________________
___________________________________________________
___________________________________________________
```

The story I was telling myself at the start of this week:

```
___________________________________________________
```

The story I'm telling myself now:

```
___________________________________________________
```

What I want to carry into week 4:

```
___________________________________________________
___________________________________________________
```

---

## Pages 31–34 — Week 4 spread (lighter — only 4 daily pages plus reflection)

### Page 31 — Week 4 cover

> *Final week*

The thing you came into this planner hoping would change — has it?

```
___________________________________________________
___________________________________________________
___________________________________________________
```

If you only completed one more page in this planner, which would you want it to be?

```
___________________________________________________
```

What's permission you're giving yourself for the rest of the month?

```
___________________________________________________
___________________________________________________
```

### Pages 32–34 — Daily pages, week 4 (3 days)

**Day 22:**
> What does "enough" look like today?

**Day 23:**
> What's a tiny rebellion against productivity you can do today?

**Day 24:**
> What's a small thing that doesn't need optimizing?

---

## Page 35 — Monthly reflection (the long one — 2 pages)

> *Find 20 minutes if you can. If not, do this in 4 sittings of 5 minutes.*

### The big questions

What were the 3 best moments of this month? (Not Instagram moments — actual best moments.)

```
1. ________________________________________________
   ________________________________________________

2. ________________________________________________
   ________________________________________________

3. ________________________________________________
   ________________________________________________
```

What were the hardest 3 moments? (Be honest. The planner won't tell.)

```
1. ________________________________________________

2. ________________________________________________

3. ________________________________________________
```

What surprised you about this season — your kid, yourself, the day-to-day?

```
___________________________________________________
___________________________________________________
___________________________________________________
___________________________________________________
```

### The pattern questions

What's something you did this month that wasn't sustainable?

```
___________________________________________________
___________________________________________________
```

What's something you almost didn't do — but it was the right call?

```
___________________________________________________
___________________________________________________
```

Where did you say yes when you meant no?

```
___________________________________________________
___________________________________________________
```

Where did you say no when you meant yes (or where you wish you'd said yes)?

```
___________________________________________________
___________________________________________________
```

### The forward question

What's the version of next month that would actually feel okay to live through?

```
___________________________________________________
___________________________________________________
___________________________________________________
___________________________________________________
```

What's one small thing you'd like to carry forward?

```
___________________________________________________
```

What's one thing you'd like to leave behind?

```
___________________________________________________
```

---

## Page 36 — A closing note

You did this.

I know you might have skipped pages. I know there might be days that look like a tornado went through the page (or no marks at all). I know there's probably a coffee ring on page 19.

That's the planner doing its job.

The point was never the pages. The point was the 60 seconds you took to ask yourself a question. The 30 seconds you spent noticing a small moment. The handful of times you wrote something honest in a quiet corner of a Tuesday.

If those things happened — even once — you've done the thing.

If they didn't happen this month — that's also okay. The planner will still be here next month. So will you.

The undated, no-deadline, no-shame nature of this thing is the entire point. Use it again when you want to. Skip it when you don't. Buy it for a friend who's tired.

Slow is a practice. It takes time. And probably more than one planner.

— *the brand*

---

*This is a planning and reflection tool. It is not therapy, medical advice, or a substitute for professional support. If you're struggling with your mental health, please reach out — the US Maternal Mental Health Hotline is 1-833-852-6262.*
