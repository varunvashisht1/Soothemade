...
| 8 | **Sales channel + sequence.** Primary channel is **Gumroad** (Merchant of Record, prices in **USD** globally, pays Indian creators via PayPal) — chosen because the owner is in India (Stripe is invite-only there; Lemon Squeezy locks India stores to INR) and the buyers are US, so USD pricing matters. Lemon Squeezy (INR) + self-hosted **Stripe** stay built but dormant for future India-geo-routing or post-incorporation. Etsy + KDP are optional *additional* channels. Sequence: conversion surface (done) → list products on Gumroad in USD → drive Pinterest traffic → **first customer** → data-driven iteration. P01 is live at $24. | 2026-05-30 | AS_BUILT §18, `STRIPE-SETUP.md` |
